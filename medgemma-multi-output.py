...
OUTPUT_DIR = Path("/workspace/llm-tests/newTest/MedGemma-27b-text-it")
TEMPLATE_PATH = Path("/workspace/llm-tests/templates/doctor-template-specialized-v2.json")
PROMPT_PATH = Path("/workspace/llm-tests/prompt-v2.txt")  # Using the new universal prompt
...
# Stop sequence for MedGemma's ChatML format
...
def generate_single_request(transcript: str, transcript_name: str) -> dict:
    ...
    full_instruction = build_full_prompt(transcript)
    
    # Format the prompt using MedGemma's required ChatML format
    formatted_prompt = f"<start_of_turn>user\n{full_instruction}<end_of_turn>\n<start_of_turn>model\n"
