#!/usr/bin/env python3
"""
Chat-to-JSON runner for llama.cpp / vLLM-style chat servers.

- Expects LLAMA_MODEL_NAME (or VLLM_MODEL_NAME) in env or .env
- Optional LLAMA_API_URL in env (defaults to http://localhost:8000/v1/chat/completions)
"""
import json
import time
import subprocess
...
OUTPUT_PATH = "/workspace/llm-tests/Output/testing/tomoko-gpt.json"
...
def generate_response_llama(system_prompt: str, template: str, transcript: str) -> str:
    ...
    try:
        resp = requests.post(LLAMA_API_URL, json=payload, headers=headers, timeout=REQUEST_TIMEOUT)
...
def main():
    ...
    logger.info(f"🚀 Processing with chat model {MODEL_NAME} @ {LLAMA_API_URL}")
    system_prompt, template, transcript = load_files()
    try:
        response = generate_response_llama(system_prompt, template, transcript)
    except Exception as e:
